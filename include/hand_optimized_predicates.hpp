...
inline int orient2d_interval(interval_number p1x,
                             interval_number p1y,
                             interval_number p2x,
                             interval_number p2y,
                             interval_number p3x,
...
inline int orient3d_filtered(double px,
                             double py,
                             double pz,
                             double qx,
                             double qy,
                             ...
                             double sz)
{
    ...
    eb  = 7.7715611723761027e-016
         * (fabs(fbdxcdy) + fabs(fcdxbdy) + fabs(fcdxady) + fabs(fadxcdy) + fabs(fadxbdy) + fabs(fbdxady));
    return ((det >= eb) - (-det >= eb));
}

inline int orient3d_interval(interval_number px,
                             interval_number py,
                             interval_number pz,
                             interval_number qx,
                             interval_number qy,
...
inline int orient3d_exact(double pdx,
                          double pdy,
                          double pdz,
                          double pax,
                          double pay,
                          ...
                          double pcz)
{
    ...
    if ((adx[0] == 0.0) && (bdx[0] == 0.0) && (cdx[0] == 0.0) && (ady[0] == 0.0) && (bdy[0] == 0.0) && (cdy[0] == 0.0)
        && (adz[0] == 0.0) && (bdz[0] == 0.0) && (cdz[0] == 0.0))
        return (det > 0) - (det < 0);

    eb   = 3.2047474274603644e-031 * pm + 1.1102230246251565e-016 * fabs(det);
    det += (adz[1] * ((bdx[1] * cdy[0] + cdy[1] * bdx[0]) - (bdy[1] * cdx[0] + cdx[1] * bdy[0]))
            + adz[0] * (bdx[1] * cdy[1] - bdy[1] * cdx[1]))
           + (bdz[1] * ((cdx[1] * ady[0] + ady[1] * cdx[0]) - (cdy[1] * adx[0] + adx[1] * cdy[0]))
              + bdz[0] * (cdx[1] * ady[1] - cdy[1] * adx[1]))
           + (cdz[1] * ((adx[1] * bdy[0] + bdy[1] * adx[0]) - (ady[1] * bdx[0] + bdx[1] * ady[0]))
              + cdz[0] * (adx[1] * bdy[1] - ady[1] * bdx[1]));
    ri = (det >= eb) - (-det >= eb);
    if (ri) return ri;

    // Filters did not work. Compute exactly...
    supo3d1(adx, ady, bdx, cdx, bdy, cdy, at_b, at_c, inv, adxt_bdy, adyt_bdx, adyt_cdx, adxt_cdy, at_blen, at_clen);

    supo3d1(bdx, bdy, cdx, adx, cdy, ady, bt_c, bt_a, inv, bdxt_cdy, bdyt_cdx, bdyt_adx, bdxt_ady, bt_alen, bt_clen);

    supo3d1(cdx, cdy, adx, bdx, ady, bdy, ct_a, ct_b, inv, cdxt_ady, cdyt_adx, cdyt_bdx, cdxt_bdy, ct_alen, ct_blen);

    bctlen = expansionObject::Gen_Sum(bt_clen, bt_c, ct_blen, ct_b, bct);
    wlen   = expansionObject::Gen_Scale(bctlen, bct, adz[1], w);
    finlen = expansionObject::Gen_Sum(finlen, fin[wh], wlen, w, fin[(!wh)]);
    wh     = !wh;
    ...
    if (adz[0] != 0.0) {
        ...
        finlen = expansionObject::Gen_Sum(finlen, fin[wh], vlen, v, fin[(!wh)]);
        wh     = !wh;
    }
    if (bdz[0] != 0.0) {
